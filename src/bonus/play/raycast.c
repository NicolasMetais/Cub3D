...
void	horizontal_cast(t_core *core)
{
	...
	if (core->rc->ra > PI)
	{
		...
		core->rc->rx = (core->rc->pl_y - core->rc->ry) * core->rc->atan + core->rc->pl_x;
		core->rc->y = -8;
...
void	vertical_cast(t_core *core)
{
	...
	if (core->rc->ra > PI / 2 && core->rc->ra < 3 * PI / 2)
	{
		...
		core->rc->ry = (core->rc->pl_x - core->rc->rx) * core->rc->ntan + core->rc->pl_y;
		core->rc->x = -8;
...
{
	core->rc->ra = core->rc->pl_angle - RAD * ((core->fov) / 2 + 1);
	if (core->rc->ra < 0)
		core->rc->ra += 2 * PI;
	if (core->rc->ra > 2 * PI)
		core->rc->ra -= 2 * PI;
	...
	while (core->rc->r < core->rc->max_r)
	{
		...
		core->rc->dist[core->rc->r] = sqrt((core->rc->rx - core->rc->pl_x) * (core->rc->rx - core->rc->pl_x) + \
		(core->rc->ry - core->rc->pl_y) * (core->rc->ry - core->rc->pl_y));
		vertical_cast(core);
		core->rc->dist2[core->rc->r] = sqrt((core->rc->rx - core->rc->pl_x) * (core->rc->rx - core->rc->pl_x) + \
		(core->rc->ry - core->rc->pl_y) * (core->rc->ry - core->rc->pl_y));
		get_good_dist(core);
		...
		if (core->rc->ca > PI)
			core->rc->ca -= 2 * PI;
		core->rc->dist3[core->rc->r] = core->rc->dist3[core->rc->r] * (cos(core->rc->ca));
		draw_3d(core);
		rays_updates(core);
	}
}
...
bool	rc_init(t_core *core)
{
	...
	core->rc->dist = gc_malloc(&core->gc, sizeof(float) * core->rc->max_r, FLOAT, "dist");
	core->rc->dist2 = gc_malloc(&core->gc, sizeof(float) * core->rc->max_r, FLOAT, "dist2");
	core->rc->dist3 = gc_malloc(&core->gc, sizeof(float) * core->rc->max_r, FLOAT, "dist3");
